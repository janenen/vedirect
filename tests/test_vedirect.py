...
class TestVEDirect(unittest.TestCase):
    ...
    def test_v_mppt(self):
        ...
        thread_slave = threading.Thread(
            target=vedir.read_data_single_callback, args=[self.cb_function]
        )
        thread_slave.start()
        ...
        self.assertDictEqual(
            self.data, VEDirect.typecast(VEDirectDeviceEmulator.data["MPPT"])
        )
    ...
    def test_emulate(self):
        ...
        self.assertEqual(
            VEDirect.typecast(VEDirectDeviceEmulator.data["MPPT"]), onerecord
        )
    ...
    def test_emulate2(self):
        ...
        self.assertEqual(
            VEDirect.typecast(VEDirectDeviceEmulator.data["MPPT"]), records[0]
        )
        self.assertEqual(
            VEDirect.typecast(VEDirectDeviceEmulator.data["MPPT"]), records[1]
        )
